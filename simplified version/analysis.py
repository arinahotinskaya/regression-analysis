...
def decimal_date(year, day):
  if calendar.isleap(year):
    ...
    n = 355
  time_event = np.trunc((year + (day - 0.5) / n) * 1e5) / 1e5
  return time_event
...
def diapason(direction): # динамическая ось У
  min_val = np.min(direction) - 2.5
  max_val = np.max(direction) + 2.5
...
def draw_time_series(count, fig, ax, pdf, interval, resolution, folder_name, name_of_station, time, direction, sigma, y, days_of_quake, V, Cm, past_markers):
  names_of_dir = ['north', 'east', 'up']
  # colors = ['green', 'red']
  colors = ['red', 'green']
  ...
  if count == len(colors):
    for i, el in enumerate(ax):
      ...
      time_pr, direction_pr, _, _, _ = dt.basic_initial(
        '/Users/arina/Desktop/Python-скрипты/Регрессионный анализ упрощенный/Результаты/Измерения 2 раза в год/ARTU/All measurements/artu_GPSRaw.neu')
      el.plot(time_pr, direction_pr[i], 'o', markersize=1, color='grey')

      # Добавляем новые маркеры и сохраняем ссылки на них
      new_markers = el.plot(time, direction[i], 'o', markersize=4, color='black')
      # past_markers.extend(new_markers)  # сохраняем ссылки на новые маркеры
  ...
  for i, el in enumerate(ax): # построение модельной кривой
    el.plot(time, y[i], color=color, linewidth=count)
    # for day in days_of_quake: # построение моментов землетрясений
    #   el.axvline(x=day, color='gray', linestyle='--', linewidth=0.5)

  # if count == 1:
  #   time_pr, direction_pr, _, _, _ = dt.basic_initial(
  #     '/Users/arina/Desktop/Python-скрипты/Регрессионный анализ упрощенный/Измерения 2 раза в год/NRIL/All measurements/nril_GPSRaw.neu')
  #   el.plot(time_pr, direction_pr[i], 'o', markersize=0.8, color='grey')
    # # Удаляем старые маркеры
    # for marker in past_markers:
    #   marker.remove()
    # past_markers.clear()
    # for i, el in enumerate(ax):
    #   new_markers = el.plot(time, direction[i], 'o', markersize=1.5, color='black')
    #   past_markers.extend(new_markers)  # сохраняем ссылки на новые маркеры

  # plt.suptitle(name_of_station.upper(), fontsize=20, fontweight='bold')  # название станции
  plt.suptitle('Красная линия - Многолетняя скорость, оцененная по непрерывному временному ряду\nЗеленая линия - Прямая, оцененная за интервал 2 года',
  ...
  return resolution, past_markers

def regression(past_markers, count, fig, ax, pdf, interval, resolution, name_of_station, folder_name, main_file, file_with_quakes=None):
  ...
  try:
    ...
    Cm_N = np.linalg.inv(np.dot(np.dot(A.transpose(), C_inv_N), A))  # Результирующая ковариационная матрица
    Cm_E = np.linalg.inv(np.dot(np.dot(A.transpose(), C_inv_E), A))
    Cm_U = np.linalg.inv(np.dot(np.dot(A.transpose(), C_inv_U), A))
    print(np.dot(A.transpose(), C_inv_N), '\n')
    print(np.dot(np.dot(A.transpose(), C_inv_N), A), '\n')
    print(np.dot(np.dot(A.transpose(), C_inv_N), A), np.eye(A.shape[1]), '\n')
    print(np.eye(A.shape[1]), '\n')
    print(Cm_N, '\n')
    print(Cm_E, '\n')
    print(Cm_U, '\n')
    ...
    resolution, past_markers = draw_time_series(count, fig, ax, pdf, interval, resolution, folder_name, name_of_station, time, direction, sigma, y, days_of_quake,
                     [x_N[1], x_E[1], x_U[1]], [Cm_N[1][1], Cm_E[1][1], Cm_U[1][1]], past_markers)
    ...
    return a, b, sgm, resolution, past_markers
